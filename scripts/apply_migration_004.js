...
async function main() {
  console.log("Applying Migration 004 to Supabase database (pooled mode, sequential execution)...");
  const migrationPath = path.join(__dirname, '..', 'supabase', 'migrations', '004_allow_custom_roles_and_notifications.sql');
  ...
  try {
    ...
    console.error("❌ Failed to apply Migration 004:", err);
    process.exit(1);
  } finally {
    await sql.end();
  }
